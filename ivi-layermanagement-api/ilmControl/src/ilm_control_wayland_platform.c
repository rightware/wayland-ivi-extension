                    ...
                    uint32_t format, uint32_t timestamp)
{
    ...
    // break the waitting in the request
...
ilm_takeScreenshot(t_ilm_uint screen, t_ilm_const_string filename)
{
    ...
    // don't accept the empty file name from input
    if (!filename) {
        fprintf(stderr, "ilm_takeScreenshot: filename is empty\n");
        return returnValue;
    }
    lock_context(ctx);
    ...
    if (ctx_scrn != NULL) {
        ...
        // create the shm buffer, using the width and height of screen
        ...
        // send the screenshot request to server, it will create a ivi_screenshot proxy
                              ...
                              t_ilm_surface surfaceid)
{
    ...
    if (ctx->wl.controller) {
        ...
        // create the shm buffer, using the width and height of surface
